...
public class BingLiChaXun_FeesDetail {
	...
	public BingLiChaXun_FeesDetail(Map infoMap) {
		// TODO Auto-generated constructor stub
		this.infoMap = infoMap;
	}
	
//	private void getMysql(String sql) throws SQLException {
//		MysqlUtil.executesql(sql);
//	}
...
//		其他费用		
	 ...
	 * @throws SQLException
	 */
	public void saveDrugFees(String response) throws IOException, SQLException {
		String type_feeds = "药品";
		JsonNode drugrows = new ObjectMapper().readTree(response).get("rows");
	
		if(drugrows.isArray()){
			String sql = "INSERT INTO binglichaxun_feeds_yp (sicode,pid,name_hos,name_yb,name_bz,ybcode,bzjx,num,price,type_bx,type_feeds,type_wg,discharge_date) VALUES ";
			String setchar="(?,?,?,?,?,?,?,?,?,?,?,?,?)";
			String s = "";
			...
			for (JsonNode drugrow : drugrows) {
				...
				Object[] value= {infoMap.get("si_code"),infoMap.get("miid"),name_hos,name_yb,name_bz,ybcode,bzjx,num,price,type_bx,type_feeds,type_wg,infoMap.get("discharge_date")};
				value_arr.add(value);
...
//			}
	...
	public void saveInspectionFees(String response) throws IOException, SQLException {
		...
		JsonNode drugrows = new ObjectMapper().readTree(response).get("rows");
		if(drugrows.isArray()){
			String sql = "INSERT INTO binglichaxun_feeds_inspection (sicode,pid, name_hos, name_yb,name_bz,num, price, type_bx, type_feeds,type_wg,discharge_date) VALUES ";
			String setchar="(?,?,?,?,?,?,?,?,?,?,?)";
			String s = "";
			List value_arr = new ArrayList<>();
			for (JsonNode drugrow : drugrows) {
				...
				s=s.concat(setchar).concat(",");
				Object[] value= {infoMap.get("si_code"),infoMap.get("miid"),name_hos,name_yb,name_bz,num,price,type_bx,type_feeds,type_wg,infoMap.get("discharge_date")};
				value_arr.add(value);
			}
			String newSql = sql.concat(s);
			MysqlUtil.executesql(newSql.substring(0,newSql.length()-1), value_arr);
	...
	public void saveCureFees(String response) throws IOException, SQLException {
		...
		JsonNode drugrows = new ObjectMapper().readTree(response).get("rows");
		if(drugrows.isArray()){
			String sql = "INSERT INTO binglichaxun_feeds_cure (sicode,pid, name_hos, name_yb,name_bz,num, price, type_bx, type_feeds,type_wg,discharge_date) VALUES ";
			String setchar="(?,?,?,?,?,?,?,?,?,?,?)";
			String s = "";
			List value_arr = new ArrayList<>();
			for (JsonNode drugrow : drugrows) {
...
public void saveMaterialsFees(String response) throws IOException, SQLException {
	...
	JsonNode drugrows = new ObjectMapper().readTree(response).get("rows");
	if(drugrows.isArray()){
		String sql = "INSERT INTO binglichaxun_feeds_materials (sicode,pid, name_hos, name_yb,name_bz,num, price, type_bx, type_feeds,type_wg,discharge_date) VALUES ";
		String setchar="(?,?,?,?,?,?,?,?,?,?,?)";
		String s = "";
		List value_arr = new ArrayList<>();
		for (JsonNode drugrow : drugrows) {
...
//    		saveMaterialsFees(getPerPages(nameValuePairList));
	...
	public String getPerPages(List<NameValuePair> nameValuePairList) throws ClassNotFoundException, URISyntaxException, IOException {
		String response = DoRequest.dopost(Constant.url+getDetail_url, nameValuePairList, MyCookieStore.readCookieStore("cookie"));
		return response;
	}
	...
	public static void main(String[] args) throws Exception {
        ...
        if(login.loginURL()){
        	...
        	BingLiChaXun_FeesDetail bc = new BingLiChaXun_FeesDetail(infoMap);
            bc.getAllFeesDetail();
        }else {
